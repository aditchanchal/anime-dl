...
# Using tr instead of requests to rotate proxy
def tor_get(url):
    req = tr.get(url)
...
def get_master_m3u8(anime_page):
    ...
    master_m3u8 = tr.get(master_m3u8_url)
...
def ts_downloader(title, resolution, ts_urls):
    ...
    download_bar = tqdm(ThreadPool(20).imap(fetch_url, ts_urls), total=len(ts_urls), desc=episode + ' ' + resolution + 'p ' + str(size / (1024 * 1024)) + 'MB')
